...
class PosljiSporociloHandler(BaseHandler):
    def post(self):
        ...
        if uporabnikovo_ime == "":
            uporabnikovo_ime = "Anonimnez"

        if uporabnikov_email == "":
            uporabnikov_email = "ni podano"

        sporocilo = Sporocilo(ime=uporabnikovo_ime, email=uporabnikov_email, tekst=uporabnikovo_sporocilo)
        sporocilo.put()
